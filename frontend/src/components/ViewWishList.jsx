...
const ViewWishList = () => {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:3060/api/wishlist/${currentUserId}`);

      console.log("Wishlist response:", response.data);

      // Assuming the response contains an array of wishlists, get the first one that matches the user ID
      const userWishlist = response.data.find((wishlist) => wishlist.userId === currentUserId);
  ...
  const fetchItemsDetails = async (itemIds) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3060/api/items`, {
        ids: itemIds,
      });
  ...
  const handleRemoveFromWishlist = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:3060/api/wishlist/${currentUserId}/${itemId}`);
  ...
  const downloadWishlistPDF = () => {
    ...
    pdf.setLineWidth(1);
    pdf.rect(padding - 10, padding - 10, pageWidth - padding * 2 + 20, pageHeight - padding * 2 + 10); // Draw rectangle for border
    ...
    itemsDetails.forEach((item, index) => {
      ...
      if (item.base64Image) {
        ...
        pdf.text(`Category: ${item.category}`, padding + 90, itemYPosition + 35);
        pdf.text(`Price: $${item.price}`, padding + 90, itemYPosition + 50);
        pdf.text(`Size: ${item.size}`, padding + 90, itemYPosition + 65);
        pdf.text(`Location: ${item.itemLocation}`, padding + 90, itemYPosition + 80);
      }
    });
  ...
  return (
    <div className='flex'>
      ...
      <div className='w-3/4 p-6'>
        ...
        <button onClick={downloadWishlistPDF} className='mt-4 bg-blue-500 text-white px-4 py-2 rounded'>
          ...
          <div className='flex flex-col space-y-4'>
            {itemsDetails.length > 0 ? (
              itemsDetails.map((item) => (
                <div key={item._id} className='border rounded p-4 mb-4 flex items-start'>
                  <img
                  ...
                  <div className='flex-grow'>
                    <span className='font-semibold cursor-pointer' onClick={() => handleItemClick(item._id)}>
                      {item.productName}
                    </span>{" "}
                    <br />
                    <span className='text-gray-700'>Category: {item.category}</span> <br />
                    <span className='text-gray-700'>Price: ${item.price}</span> <br />
                    <span className='text-gray-700'>Size: {item.size}</span> <br />
                    <span className='text-gray-700'>Location: {item.itemLocation}</span>
                  </div>
                  <button onClick={() => handleRemoveFromWishlist(item._id)} className='text-red-500 ml-4'>
                    <AiFillHeart size={24} color='red' /> {/* Filled red heart */}
                  </button>
                </div>
              ))
            ) : (
