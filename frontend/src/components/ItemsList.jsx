...
const ItemsList = () => {
  ...
  const filterItems = () => {
    ...
    if (selectedCategory && selectedCategory !== "All") {
      result = result.filter((item) => item.category.includes(selectedCategory));
    }
  ...
  const handleWishlistToggle = async (item) => {
    ...
    if (isInWishlist(item)) {
      ...
      updatedWishlist = wishlist.filter((wishItem) => wishItem._id !== item._id);
    } else {
    ...
    <div className='flex'>
      ...
      <div className='w-1/6 pl-16 pt-10 border-r'>
        ...
        <div className='mb-6'>
          ...
          <ul>
            <li>
              <button className={`block mb-2 ${selectedCategory === "All" ? "text-orange-500" : ""}`} onClick={() => handleCategoryChange("All")}>
                All
              </button>
            </li>
            {/* Unique categories */}
            {[...new Set(items.map((item) => item.category))].map((category) => (
              <li key={category}>
                <button
                  className={`block mb-2 ${selectedCategory === category ? "text-orange-500" : ""}`}
        ...
        <div className='mb-6'>
          ...
          <div className='mb-2'>
            ...
            />
            <input type='number' name='maxPrice' value={maxPrice} onChange={handlePriceChange} placeholder='Max Price' className='w-50 border p-2' />
          </div>
        </div>
          ...
          <div className='grid grid-cols-1 gap-6'>
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => (
                <div key={item._id} className='bg-gray-100 p-4 rounded shadow-sm flex items-start justify-between relative'>
                  {/* Image on the left */}
                  <div className='w-1/4'>
                    <img
                      src={item.imageUrl}
                  ...
                  <div className='w-2/3 ml-4 flex flex-col'>
                    <h3 className='text-lg font-semibold mt-2 cursor-pointer' onClick={() => handleItemClick(item._id)}>
                      {item.productName}
                    </h3>
                    <p className='text-gray-700'>Category: {item.category}</p>
                    <p className='text-gray-700'>Price: ${item.price}</p>
                    <p className='text-gray-700'>Size: {item.size}</p>
                    <p className='text-gray-700'>Location: {item.shopLocation}</p>

                    {/* Render heart icon only if the item is not in the wishlist */}
                    {!isInWishlist(item) && (
                      <button onClick={() => handleWishlistToggle(item)} className='absolute top-2 right-2 text-red-500'>
