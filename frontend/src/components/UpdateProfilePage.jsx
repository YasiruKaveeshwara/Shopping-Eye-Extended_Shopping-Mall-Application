...
const UpdateProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:3060/api/auth/me", {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "http://localhost:3060/api/auth/update",
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      try {
        const token = localStorage.getItem("token");
        await axios.delete("http://localhost:3060/api/auth/delete", {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      <div className='flex-grow min-h-screen bg-gray-100 p-8'>
        <div className='max-w-4xl mx-auto bg-white p-8 shadow-lg rounded-lg'>
          <div className='flex items-center mb-6'>
            ...
            <h2 className='text-2xl font-bold text-center flex-grow'>Update Profile</h2>
          </div>
          {error && <p className='text-red-500 text-center'>{error}</p>}
          {success && <p className='text-green-500 text-center'>{success}</p>}
          {deleteStatus && <p className='text-red-500 text-center'>{deleteStatus}</p>}
          ...
          <form onSubmit={handleUpdate} className='grid grid-cols-2 gap-6'>
            ...
            <div className='col-span-2 flex justify-between mt-6 gap-5'>
              ...
              <button type='button' onClick={handleDelete} className='bg-red-500 text-white w-full py-2 rounded hover:bg-red-600'>
                Delete Profile
              </button>
            </div>
          </form>
