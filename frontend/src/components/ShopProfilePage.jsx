...
const ShopProfilePage = () => {
  ...
  useEffect(() => {
    const fetchShop = async () => {
      try {
        const response = await fetch(`http://localhost:3060/api/shops/${id}`);
        if (!response.ok) {
    ...
    const fetchAllProducts = async () => {
      try {
        const response = await fetch(`http://localhost:3060/api/items`);
        if (!response.ok) {
    ...
    const fetchFeedbacks = async () => {
      try {
        const response = await fetch(`http://localhost:3060/api/feedbacks/${id}`);
        if (!response.ok) {
    ...
    const filterProductsByShop = (products) => {
      if (shop) {
        const filtered = products.filter((product) => product.shopName === shop.shopName);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:3060/api/feedbacks`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className='min-h-screen bg-gray-100'>
      <div className='container p-6 mx-auto'>
        {/* Header Section */}
        <div className='flex items-center justify-between p-4 mb-6 bg-white shadow-md'>
        ...
        {/* Store Navigation */}
        <div className='flex justify-around p-2 mb-4 bg-gray-100'>
          <button onClick={handleShowStore} className={`font-semibold ${!showProducts ? "text-orange-500" : "text-gray-500"}`}>
            Store
          </button>
          <button onClick={handleShowProducts} className={`font-semibold ${showProducts ? "text-orange-500" : "text-gray-500"}`}>
            Products
          </button>
        </div>
        ...
        {!showProducts ? (
          <div className='p-6 mb-6 bg-white rounded shadow-sm'>
            {/* Shop Details Section */}
            <p className='mb-1 text-gray-700'>Location: {shop.location}</p>
            <p className='mb-1 text-gray-700'>Category: {shop.shopCategory}</p>
            <p className='mb-1 text-gray-700'>Phone: {shop.phone}</p>
            <p className='mb-1 text-gray-700'>Email: {shop.email}</p>

            {/* Ratings and Feedback Section */}
            <div className='mt-10'>
              <h2 className='mb-4 text-2xl font-semibold'>Ratings & Feedback</h2>
              <div>
                {/* <h3 className="text-xl font-semibold">Overall Ratings:</h3>
                <div className="flex items-center">
                  {feedbacks.length > 0 ? (
              ...
              <div className='mt-4'>
                ...
                <ul>
                  {feedbacks.length > 0
                    ? feedbacks.map((fb) => (
                        <li key={fb._id} className='p-4 mb-6 bg-white border border-gray-300 rounded-lg shadow-sm'>
                          <div className='items-start'>
                            <p className='mr-2 font-medium'>{fb.user}</p>
                            <div className='flex' style={{ fontSize: "0.5rem" }}>
              ...
              {/* Feedback Form */}
              <div className='mt-10'>
                <h2 className='mb-4 text-2xl font-semibold'>Submit Feedback</h2>
                <form onSubmit={handleSubmit}>
                  ...
                  <div className='mb-4'>
                    ...
                    <textarea value={feedback} onChange={handleFeedbackChange} className='w-full p-2 border border-gray-300 rounded' rows='4' />
                  </div>
                  <div className='mb-4'>
                    <label className='inline-flex items-center'>
                      <input type='checkbox' checked={isAnonymous} onChange={handleAnonymousChange} className='form-checkbox' />
                  ...
                  {!isAnonymous && (
                    <div className='mb-4'>
                      ...
                      <input type='text' value={userName} onChange={handleUserNameChange} className='w-full p-2 border border-gray-300 rounded' />
                    </div>
                  )}
                ...
                {submitted && <div className='mt-4 text-green-500'>Feedback submitted successfully!</div>}
              </div>
            </div>
          </div>
        ) : (
            ...
            <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4'>
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product) => (
                  <div key={product._id} className='p-4 bg-gray-100 rounded shadow-sm'>
                    <img src={product.imageUrl} alt={product.productName} className='object-cover rounded w-50 h-50' />
                    ...
                    <p className='text-gray-700'>Location: {product.itemLocation}</p>
