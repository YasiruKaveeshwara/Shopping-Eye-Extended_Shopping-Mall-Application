...
const ItemPage = () => {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const response = await fetch(`http://localhost:3060/api/items/${id}`);
      ...
      {item ? (
        <>
          <div className='flex flex-col items-center lg:items-start lg:w-1/2 p-4'>
            {item.imageUrl && <img src={item.imageUrl} alt={item.productName} className='w-90 h-90 object-cover mb-4 ml-20' />}
            <div className='flex justify-center lg:justify-start space-x-2 mb-4'>
              ...
              {item.thumbnailUrls &&
                item.thumbnailUrls.map((url, index) => (
                  <img key={index} src={url} alt={`Thumbnail ${index + 1}`} className='w-16 h-16 object-cover border' />
                ))}
            </div>
          </div>
          ...
          <div className='flex flex-col lg:w-1/2 p-4'>
            ...
            <p className='text-2xl font-semibold text-orange-600 mb-2'>Rs. {item.price}</p>
            ...
            <p className='text-xl mb-2'>
              <span className='font-bold'>Shop Location:</span> {item.shopLocation}
            </p>
